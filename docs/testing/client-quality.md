...
| Hypium 本地业务规则 | 通过 | Hvigor `test` 构建成功；47 个用例全部通过，Failure 0、Error 0，包含注册与错误映射、门票状态机、预约冲突、路线节点顺序、上海时间统一格式，以及历史数据展示兼容。 |
...
2. 门票报价后创建待支付订单，验证“取消订单”会立即变为“已取消”并释放余票；另建订单完成演示支付，在订单卡片显示的退票截止时间前填写退款原因并验证“已退款”。
...
6. 餐住重复时段会给出明确冲突原因；修改日期或时段后可重新提交，确认或取消后状态与按钮立即更新。
7. 商城结算、积分兑换、客服、同行协作和无障碍偏好可达且输入错误可恢复。
8. 首页“应急助手”能进入“我的 → 离线/应急”；首页各快捷服务均有明确目标页面，页面不再显示无目标的“全部服务”。
9. 离线包冷启动、待同步 outbox、SOS Demo、数字护照和绿色积分明确标注状态，不把演示能力呈现为真实救援派单。
