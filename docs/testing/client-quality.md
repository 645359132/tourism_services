...
| Hypium 本地业务规则 | 通过 | Hvigor `test` 构建成功；45 个用例全部通过，Failure 0、Error 0，包含注册字段、重复用户名错误映射、历史英文行程的中文展示兼容、模拟器连接提示、路线节点顺序，以及住宿冲突和预约状态刷新。 |
...
2. 门票报价、库存变化、支付状态、二维码入口和退改入口布局稳定；餐住重复时段会给出明确提示，确认或取消后状态与按钮立即更新。
