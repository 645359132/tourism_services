...
| 服务端测试/覆盖率 | `uv run pytest`；配置内置 branch coverage 和 `fail_under=70` | 159 passed，覆盖率 72.89%，含签名报价/退款库存、演出真实重叠、商城余量、队列 CAS 与 PostgreSQL 离线 DDL 门禁 | 通过 |
...
| Hypium 本地测试 | `entry@default` Hvigor test | 49 passed，Failure 0，Error 0 | 通过 |
...
- canonical `uv run pytest` 已通过 159 个测试，分支覆盖率 72.89%；`server/pyproject.toml` 自动追加
...
`file:` 或本机路径。Hypium `entry@default` 已执行 49 个用例，Failure 0、Error 0；
