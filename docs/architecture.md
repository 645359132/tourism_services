...
3. **Identity and authorization.** Public registration creates only a normalized `tourist` identity and an immediately usable token session; callers cannot choose a role ([registration route](../server/app/api/routes/auth.py#L64), [registration service](../server/app/services/auth.py#L60)). Route dependencies then load a bearer access token and enforce tourist, merchant, support, or admin roles; `admin` is the deliberate superuser bypass ([authorization dependencies](../server/app/api/dependencies/auth.py#L20)).
...
- Passwords use Argon2id. Registration trims display names, normalizes usernames to lowercase, validates the public credential policy, resolves username races through the database uniqueness constraint, and returns a token pair only after the user, tourist role, and refresh session commit ([request schema](../server/app/schemas/auth.py#L25), [registration service](../server/app/services/auth.py#L60)). Validation errors from credential routes omit submitted input values.
- Access, refresh, and ticket-QR JWTs require the configured algorithm, issuer, audience, type, timestamps, subject, and JTI ([security primitives](../server/app/core/security.py#L15)). Refresh tokens rotate through persisted families; reuse or invalid rotation revokes the family ([auth service](../server/app/services/auth.py#L209)). Access tokens remain short-lived and role checks query the active user. Registration, login, and refresh share the IP-bound authentication rate-limit bucket.
